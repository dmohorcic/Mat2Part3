source("examples/HMC.r")
source("examples/Multiplot.r")
library(ggplot2)
library(numDeriv)
library(coda)
library(grid)
...
samples <- NULL
pdf(paste("examples/trajectories-ex02.pdf",sep=""), width = 9, height = 3)
for (i in 1:m) {
  print(i)
...
pdf(paste("examples/densities-ex02.pdf",sep=""), width = 8, height = 4)
# true density
x <- seq(-25,25,0.2)
x0 <- expand.grid(x,x)
y <- apply(x0,1,minus_logf)
